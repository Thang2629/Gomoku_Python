...
class Computer(tk.Frame):
    ...
    def is_win(self,board):
        ...
        black = self.score_of_col(board,'b')
        white = self.score_of_col(board,'w')
    ...
    def click(self,x,y):
        ...
        if not self.is_in(board, y, x):
            return
        
        if board[y][x] == ' ':
            
            self.draw_stone(x,y,colors['b'])
            board[y][x]='b'
            
            move_history.append((x, y))
            
            game_res = self.is_win(board)
            ...
            ay,ax = self.best_move(board,'w')
            self.draw_stone(ax,ay,colors['w'])
            board[ay][ax]='w'    
    ...
    def initialize(self,size,can):
        
        global win,board,screen,colors, move_history
        
        move_history = []
        win = False
        board = self.make_empty_board(size)

        screen = turtle.Screen(can)
        screen.onclick(self.click)
    ...
    def draw_stone(self,x,y,colturtle):
